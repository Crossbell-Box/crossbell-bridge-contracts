...
contract MiraTokenTest is Test {
    ...
    function testOperatorSendFail() public {
        uint256 amount = 1 ether;
        ...
        // alice authorizeOperator for bob
        vm.prank(alice);
        token.authorizeOperator(bob);
        // bob transfers tokens from account of alice
        vm.expectRevert(abi.encodePacked("transfer is blocked"));
        vm.prank(bob);
        token.operatorSend(alice, carol, amount, "", "");
    ...
    function testBurnFail() public {
        ...
        // alice
        vm.prank(alice);
        vm.expectRevert(abi.encodePacked("burn is not allowed"));
        token.burn(amount, "");
